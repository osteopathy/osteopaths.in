...
import { serviceProviderAppointmentRequestTable, serviceProviderAppointmentTable, serviceProviderDateWiseScheduleTable, WithDrawReasons } from "$lib/database/schema";
import { db } from "$lib/database";
import { and, asc, eq, gte, not } from "drizzle-orm";
import { fail, message, superValidate } from "sveltekit-superforms";
import { zod } from "sveltekit-superforms/adapters";
import { bookAppointmentSchema, createRequestSchema, scheduleSchema, unbookAppointmentSchema, updateRequestSchema, withdrawRequestSchema } from "./schema";
...
export const load: PageServerLoad = async (event) => {
	...
	const dateschedule = await db.query.serviceProviderDateWiseScheduleTable.findMany({
		where: and(eq(serviceProviderDateWiseScheduleTable.serviceProviderId, service_provider_id), gte(serviceProviderDateWiseScheduleTable.date, new Date())),
		orderBy: [
		...
		with: {
			requests: {
				...
				where: and
					(
						not(
							eq(serviceProviderAppointmentRequestTable.status, "withdrawn")),
						not
							(eq(serviceProviderAppointmentRequestTable.status, "accepted"))
					)
			}
		},
	})
...
export const actions: Actions = {
	newschedule: async (event) => {
		...
		if (result) {
			await db.update(serviceProviderDateWiseScheduleTable).set({
	...
	unbookappointment: async (event) => {
		...
		await db.batch([
			db.delete(serviceProviderAppointmentTable).where(eq(serviceProviderAppointmentTable.id, form.data.appointment_id)),
	...
	update: async (event) => {
		...
		const service_provider_id = form.data.service_provider_id ?? event.params.service_provider_id;
		await db.update(serviceProviderAppointmentRequestTable).set({
			userId: event.locals.user.id,
			serviceProviderId: service_provider_id,
			date: form.data.date,
			startAt: form.data.start_at,
